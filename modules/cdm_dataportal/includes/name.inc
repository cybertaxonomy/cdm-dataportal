...
function render_taxon_or_name($taxon_name_or_taxon_base, $name_link = NULL, $reference_link = NULL,
  $show_taxon_name_annotations = true, $showRegistrations = true, $is_type_designation = false, $skip_render_template_parts = [], $is_invalid = false) {

  $is_doubtful = false;
  $taxon_base = null;
  $nom_status_fkey = null; // FootNoteKey
  $nom_status_additional_source_fkey = null; //FootNoteKey for additional source
  if(isset($taxon_name_or_taxon_base->class) && ($taxon_name_or_taxon_base->class == 'Taxon' || $taxon_name_or_taxon_base->class == 'Synonym')){
    ...
    } else if (isset($taxon_name->taggedLabel)){
      $tagged_title = $taxon_name_or_taxon_base->taggedLabel;
    }else {
  ...
  */
    ...
    if (isset($nom_status_tagged_text[0])) {
        ...
        foreach ($nom_status_tagged_text as &$tt){
         if($tt->type == 'nomStatus'&& isset($tt->entityReference)) {
           ...
           if (isset_not_empty($nom_status->source) ){
               $nom_status_additional_source_fkey = handle_name_relationship_as_footnote($nom_status, false);
           }
  ...
  if (isset($renderTemplate['secReferencePart'])){
    if(isset($secref_tagged_text[1])){
      ...
      $footnote_markup = '';
      $secRef_markup = cdm_tagged_text_to_markup($secref_name_inSource_tagged_text);
        if (isset($taxon_base->secSource) && variable_get(CDM_SYNONYMY_ACCEPTED_TAXON_SEC_FOOTNOTE) == CDM_SEC_WITH_FOOTNOTE ) {
            $footnote_markup .= '<span class="sec_reference">' . cdm_reference_markup($taxon_base->secSource->citation) . '</span>';

            if (isset($taxon_base->secSource->citation) && isset($taxon_base->secSource->citationMicroReference)) {
                $footnote_markup .= (isset_not_empty($taxon_base->secSource->citationMicroReference) ? ': <span class="reference_detail">' . $taxon_base->secSource->citationMicroReference . '</span>' : '') ;
            }
            if ($footnote_markup) {
                $foot_note_key_secRef = FootnoteManager::addNewFootnote(RenderHints::getFootnoteListKey(), $footnote_markup);
            }
        }
        if (isset($taxon_base->secSource) && variable_get(CDM_SYNONYMY_ACCEPTED_TAXON_SEC_FOOTNOTE) == CDM_SEC_INTERNAL_LINK) {

            $icon_link = cdm_internal_link(path_to_reference($taxon_base -> secSource -> citation ->uuid));
            $secRef_markup = l($secRef_markup, path_to_reference($taxon_base -> secSource -> citation ->uuid), array(
                'attributes' => array(
                    "class" => "sec_reference",
                ),
                'absolute' => TRUE,
  ...
  if(isset($foot_note_key_secRef)){
    // the nomenclatural status footnote key refers to the source citation
    $annotations_and_sources->addFootNoteKey($foot_note_key_secRef);
  }
  if($nom_status_fkey){
    // the nomenclatural status footnote key refers to the source citation
    $annotations_and_sources->addFootNoteKey($nom_status_fkey);
  }
  if(isset($nom_status_additional_source_fkey)){
    // the nomenclatural status footnote key refers to the source citation
    $annotations_and_sources->addFootNoteKey($nom_status_additional_source_fkey);
  }
  if ($show_taxon_name_annotations ) {
    ...
    if (isset($taxon_name) && !isset($taxon_name-> nameLabel)) { //it is no dto
...
/**
 * Renders the markup for a CDM taxon dto instance.
 *
 * The layout of the name representation is configured by the
 * part_definitions and render_templates (see get_partDefinition() and
 * get_nameRenderTemplate())
 *
 * @param $taxon_name_or_taxon_base_dto
 *    A cdm TaxonBase or TaxonName DTO entity
 * @param $name_link
 *    URI to the taxon, the path provided by path_to_taxon() must be processed
 *    by url() before passing to this method
 * @param $reference_link
 ...
 *  The markup for a taxon name.
 */
function render_new_taxon_or_name($taxon_name_or_taxon_base_dto, $name_link = NULL, $reference_link = NULL,
                              $show_taxon_name_annotations = true, $showRegistrations = true, $is_type_designation = false, $skip_render_template_parts = [], $is_invalid = false)
{
    ...
    $taxon_base = null;
    $nom_status_fkey = null; // FootNoteKey
    //if($taxon_name_or_taxon_base_dto->class == 'Taxon' || $taxon_name_or_taxon_base_dto->class == 'Synonym'){
    $taxon_base = $taxon_name_or_taxon_base_dto;
    if (isset($taxon_name_or_taxon_base_dto->taggedName)) {
        $tagged_name = $taxon_name_or_taxon_base_dto->taggedName;
    } else if ($taxon_name_or_taxon_base_dto->entity->name->taggedName) {
        $tagged_name = $taxon_name_or_taxon_base_dto->entity->name->taggedName;
    } else {
        $taxon_name = cdm_ws_get(CDM_WS_TAXON . '/$0/name', array($taxon_name_or_taxon_base_dto->uuid));
    }
    if (isset($taxon_name_or_taxon_base_dto->doubtful)) {
        $is_doubtful = $taxon_name_or_taxon_base_dto->doubtful;
    }

    // use the TaxonBase.tagged_title so we have the secRef
    if (isset($taxon_name_or_taxon_base_dto->taggedLabel)) {
        $tagged_title = $taxon_name_or_taxon_base_dto->taggedLabel;
    } else if ($taxon_name_or_taxon_base_dto->entity->taggedTitle) {
        $tagged_title = $taxon_name_or_taxon_base_dto->entity->taggedTitle;
    } else {
        $tagged_title = $taxon_name_or_taxon_base_dto;
    }


    $renderTemplate = get_nameRenderTemplate(RenderHints::getRenderPath(), $name_link, $reference_link);
    if (isset_not_empty($taxon_base->entity) && isset_not_empty($taxon_base->entity->name)) {
        $partDefinition = get_partDefinition($taxon_base->entity->name->nameType);
    } else {
        $partDefinition = get_partDefinition($taxon_name_or_taxon_base_dto->nameType);
    }
    ...
    if (isset_not_empty($skip_render_template_parts)) {
        foreach ($skip_render_template_parts as $part) {
            unset($renderTemplate[$part]);
        }
    }
    ...
    $secref_name_inSource_tagged_text = tagged_text_extract_secReference_name_in_source($tagged_title);
    // taxon names will have the nomenclatural reference in the tagged full title:
    $nomref_tagged_text = tagged_text_extract_reference($tagged_name);
    $nom_status_tagged_text = tagged_text_extract_nomstatus($tagged_name);
    $appended_phrase_tagged_text = array(); // this is filled later
    ...
    if ($is_valid_tagged_title) {
        ...
        if (!(($hasNamePart_with_Authors) || ($hasNameAuthorPart_with_Authors))) {
            ...
            */
            ...
            // Remove all authors and everything behind.
            $taggedNameNew = array();
            $nameInSourceActive = false;
            foreach ($taggedName as $element) {
                if ($element->type != 'authors' && $element->type != 'nameInSourceSeparator' && !$nameInSourceActive) {
                    $taggedNameNew[] = $element;
                } else if ($element->type == 'nameInSourceSeparator' && !$nameInSourceActive) {
                    $nameInSourceActive = true;
                }else if ($element->type == 'nameInSourceSeparator' && $nameInSourceActive){
                    $nameInSourceActive = false;
                }else {
                    $lastAuthorElementString = $element->text;
        ...
        $name = '<span class="' . "TaxonName" . '">' . $doubtful_marker_markup . $name_encasement . cdm_tagged_text_to_markup($taggedName) . $name_encasement . '</span>';
    }
    else {
        // use titleCache instead
        $name = '<span class="' . $taxon_name->class . '_titleCache">' . $doubtful_marker_markup . $name_encasement . $taxon_name->titleCache . $name_encasement . '</span>';
    ...
    */
    ...
    if (isset($renderTemplate['referencePart']) && !$is_type_designation) {
        $registration_markup = "";
        if ($showRegistrations){
            $registrations = cdm_ws_get(CDM_WS_NAME, array($taxon_base->nameUuid, "registrations"));
            $registration_markup = render_registrations($registrations);
        }
       //
        ...
        // [Eckhard]:"Komma nach dem Taxonnamen ist grundsätzlich falsch,
        // Komma nach dem Autornamen ist überall dort falsch, wo ein "in" folgt."
       if (isset($renderTemplate['referencePart']['reference']) && isset($taxon_base->sourceString)) {

          // if(isset($taxon->sourceString)){
                // drupal message for debugging of #9599
                // drupal_set_message("reference/$0/nomenclaturalCitation' must no longer be used (type: "
                //  . $taxon_name_or_taxon_base->class . (isset($taxon_name_or_taxon_base->type) ? " - " +$taxon_name_or_taxon_base->type : ""  . ")"), "error");
                $citation = $taxon_base->sourceString;
                // Find preceding element of the reference.
                $precedingKey = get_preceding_contentElementKey('reference', $renderTemplate);
                if (str_beginsWith($citation, ", in")) {
                    $citation = substr($citation, 2);
      ...
      /* } else {
                // this is the case for taxon names
                $options['html']= true;
                $referenceArray['#html'] = cdm_tagged_text_to_markup($nomref_tagged_text, $skiptags = array(), $tag = 'span', $options). $registration_markup;
       }*/
    ...
    if(isset($renderTemplate['statusPart']['status'])){
        if (isset($nom_status_tagged_text[0])) {
            ...
            foreach ($nom_status_tagged_text as &$tt){
                if($tt->type == 'nomStatus'&& isset($tt->entityReference)) {
                    //$nom_status = cdm_ws_get(CDM_WS_NOMENCLATURALSTATUS, array($tt->entityReference->uuid));
                    $nom_status = $nom_status_tagged_text;
                    $nom_status_fkey = handle_nomenclatural_status_as_footnote($nom_status);
                    $tt_to_markup_options['html'] = true;
                }
            }
    ...
    if (isset($renderTemplate['secReferencePart'])){
        ...
        if(isset($secref_tagged_text[1])) {
            ...
            $footnote_markup = '';
            if (isset_not_empty($secref_name_inSource_tagged_text)){
                $secRef_markup = cdm_tagged_text_to_markup($secref_name_inSource_tagged_text);
            }
            $foot_note_key_secRef = "";
            if (isset($secref_tagged_text[1]->entityReference) && variable_get(CDM_SYNONYMY_ACCEPTED_TAXON_SEC_FOOTNOTE) == CDM_SEC_WITH_FOOTNOTE) {
                $footnote_markup .= '<span class="sec_reference">' . cdm_reference_markup($secref_tagged_text[1]->entityReference) . '</span>';
//TODO: handle microReference
                //if (isset($secref_tagged_text[1]->entityReference) ){//&& isset($taxon_base->secSource->citationMicroReference)) {
                //    $footnote_markup .= (isset_not_empty($taxon_base->secSource->citationMicroReference) ? ': <span class="reference_detail">' . $taxon_base->secSource->citationMicroReference . '</span>' : '');
                //}
                if ($footnote_markup) {
                    $foot_note_key_secRef = FootnoteManager::addNewFootnote(RenderHints::getFootnoteListKey(), $footnote_markup);
                    if(isset($foot_note_key_secRef)){
                        // the nomenclatural status footnote key refers to the source citation
                        $annotations_and_sources->addFootNoteKey($foot_note_key_secRef);
                        $secRef_markup.= $annotations_and_sources->footNoteKeysMarkup();
                    }
                }
            }
            if (isset($secref_tagged_text[1]->entityReference) && variable_get(CDM_SYNONYMY_ACCEPTED_TAXON_SEC_FOOTNOTE) == CDM_SEC_INTERNAL_LINK) {
            ...
            /*$annotations_and_sources = handle_annotations_and_sources($taxon_base->secSource);
    ...
    $protologue_links = [];
    if (array_setr('description', TRUE, $renderTemplate)) {
        $external_links = $taxon_name_or_taxon_base_dto->protologues;
        //$external_links = cdm_ws_get(CDM_WS_NAME_PROTOLOGUE_LINKS, $taxon_name->uuid);
        if($external_links){
            foreach ($external_links as $link) {
                if (!empty($link->javaUri)) {
                    // for the link see also cdm_external_uri()
                    $protologue_links[] = l(font_awesome_icon_markup('fa-book'), $link->javaUri, ['html' => true]);
                }
            }
        }
        ...
        $additional_pub_markup = '';
        $descriptions = $taxon_name_or_taxon_base_dto-> nameFacts;
       //$descriptions = cdm_ws_get(CDM_WS_PORTAL_NAME_DESCRIPTIONS, $taxon_name->uuid);
        if($descriptions) {
            foreach ($descriptions-> items as $description) {
                if (!empty($description)) {
                    $additional_citations = [];
                    $additional_data = [];
                    //if ($description->label == "Additional Publication") {
                        foreach ($description->facts->items as $description_element) {
                            if (isset($description_element->typedLabel) ) {
                                RenderHints::setAnnotationsAndSourceConfig(synonymy_annotations_and_source_config());
                                $annotations_and_sources = handle_new_annotations_and_sources($description_element);
                                // synonymy_annotations_and_source_config() has 'sources_as_content' => FALSE, so no need to handle inline sources here
                               /* $element_media = cdm_new_description_element_media(
                                    $description_element,
                                    [
                                        'application/pdf',
                                        'image/png',
                                ...
                                */
                                if ($description->label == "Additional Publication") {
                                    $additional_citations[] = ' [& ' . trim($description_element->typedLabel[0]->label) . //$element_media .
                                        $annotations_and_sources->footNoteKeysMarkup() . ']';
                                } else {
                                    $additional_data[] = ' [' . trim($description_element->typedLabel[0]->label) . //$element_media .
                                        $annotations_and_sources->footNoteKeysMarkup() . ']';
                                }
                            }
                        }
    ...
    $out .= '<span class="' . html_class_attribute_ref($taxon_name_or_taxon_base_dto)
     //   . '" data-cdm-ref="/name/' . $taxon_name_or_taxon_base_dto->uuid . '" data-cdm-render-path="' . RenderHints::getRenderPath() .'">';
     . '" data-cdm-ref="/name/' . $taxon_name_or_taxon_base_dto->uuid . '">';
    foreach ($renderTemplate as $partName => $part) {
        $separator = '';
        $partHtml = '';
        $uri = FALSE;
    ...
    if ($show_taxon_name_annotations) {
       $annotations_and_sources = handle_new_annotations_and_sources($taxon_name_or_taxon_base_dto,
            null, null, $annotations_and_sources);
    }
    $out .= $annotations_and_sources->footNoteKeysMarkup();
...
function render_new_name_for_name_rels($taggedLabel, $taxon_name_uuid, $name_type = null, $name_link = NULL, $reference_link = NULL,
                                  $show_taxon_name_annotations = true, $is_type_designation = false, $skip_render_template_parts = [], $is_invalid = false)
{
    ...
    $renderTemplate = get_nameRenderTemplate(RenderHints::getRenderPath(), $name_link, $reference_link);
    $currentTaxon = cache_get('current_taxon');

    $partDefinition = get_partDefinition($currentTaxon->data->nameType);
    ...
    $is_valid_tagged_title =
        isset($taggedLabel)
        && is_array($taggedLabel)
        && isset($taggedLabel[0]->text)
        && is_string($taggedLabel[0]->text)
        && $taggedLabel[0]->text != ''
        && isset($taggedLabel[0]->type);
    $lastAuthorElementString = FALSE;
    ...
    $out .= '<span class="' . "TaxonName"
     //   . '" data-cdm-ref="/name/' . $taxon_name_uuid . '" data-cdm-render-path="' . RenderHints::getRenderPath() .'">';
     . '" data-cdm-ref="/name/' . $taxon_name_uuid . '">';
    foreach ($renderTemplate as $partName => $part) {
        $separator = '';
        $partHtml = '';
        $uri = FALSE;
  ...
  if(!empty($name_type_designations)){
    ...
    foreach($name_type_designations as $name_type_designation){
      ...
      elseif (isset($name_type_designation->typeName)) {
        ...
        $out .= ': ' . render_taxon_or_name($name_type_designation->typeName, $link_to_name_page, $referenceUri, TRUE, true, TRUE);
      }
...
function render_specimen_typedesignation_dto($specimen_typedesignation_dtos, $show_type_specimen = FALSE, $enclosing_tag = 'ul', $element_tag = 'li'){
  ...
  if (!empty($specimen_typedesignation_dtos)) {
    ...
    foreach ($specimen_typedesignation_dtos as $std_dto) {
      ...
      $names = "";
      if (isset_not_empty($std_dto->names)){
            foreach ($std_dto->names as $name){
                $names .= l($name->label, path_to_name($name->uuid))  ;
                $names .= "; ";
          }
      }
      $out .= type_designation_dto_status_label_markup($std_dto) . $type_citation_markup . " of ". $names;
  ...
  foreach($type_entity_refs_by_status as $type_status => $name_type_entityRefs){
    foreach ($name_type_entityRefs as $name_type_entity_ref){
      ...
      RenderHints::setAnnotationsAndSourceConfig([
        // these settings differ from those provided by annotations_and_sources_config_typedesignations()
        // TODO is this by purpose? please document the reason for the difference
           'sources_as_content' => true, // as footnotes
           'link_to_name_used_in_source' => false,
           'link_to_reference' => true,
 ...
 * @see \get_other_name
 */
function name_relationship_new_markup($name_rel, $current_taxon_uuid, $name_type, $show_name_cache_only = false){

    $relationship_markup = null;
    //taggedText
    $other_name = $name_rel->nameLabel;

    //cdm_load_tagged_full_title($other_name);

    $highlited_synonym_uuid = isset ($name_rel->nameUuid) ? $name_rel->nameUuid : '';
    if($show_name_cache_only){
        $relationship_markup = l(
            '<span class="' . html_class_attribute_ref($other_name) . '"">' . cdm_tagged_text_to_markup($other_name) . '</span>',
            path_to_name($name_rel->nameUuid, $current_taxon_uuid, $highlited_synonym_uuid, false),
            array('html' => true)
        );
        $annotations_and_sources = handle_annotations_and_sources($other_name);
        $relationship_markup .= $annotations_and_sources->footNoteKeysMarkup();
    } else {
        $relationship_markup = render_new_name_for_name_rels($other_name, $name_rel-> nameUuid, $name_type,
            url(path_to_name($name_rel->nameUuid, $current_taxon_uuid, $highlited_synonym_uuid, false))
        );
    }
 ...
 * @ingroup Compose
 */
function compose_new_name_relationships_inline($name_relations, $current_taxon_uuid, $name_type, $suppress_if_current_name_is_source = true) {
    ...
    $item_prefix = '<span class="item">';
    $item_suffix = '</span> ';
    $render_array = compose_new_name_relationships($name_relations, $name_rel_type_filter,  $current_taxon_uuid, $name_type, $list_prefix, $list_suffix, $item_prefix, $item_suffix);
                                    ...
                                    $list_prefix, $list_suffix, $item_prefix, $item_suffix)
{
  ...
  if ($name_relations) {
    ...
    foreach ($name_relations as $name_rel) {
      $rel_type_uuid = isset($name_rel->type->uuid)?$name_rel->type->uuid:null;
 ...
 * @ingroup Compose
 */
function compose_new_name_relationships($name_relations, $name_rel_type_filter, $current_taxon_uuid, $name_type,
                                    $list_prefix, $list_suffix, $item_prefix, $item_suffix)
{
    $non_nec_name_reltype_uuids = array(UUID_NAMERELATIONSHIPTYPE_LATER_HOMONYM,
        UUID_NAMERELATIONSHIPTYPE_TREATED_AS_LATER_HOMONYM,
    ...
    if ($name_relations) {
        ...
        $orthographic_variants = array();
        foreach ($name_relations as $name_rel) {
            $rel_type_uuid = $name_rel->relTypeUuid;
            $is_inverse_relation = $name_rel->inverse;
            if ((!$is_inverse_relation && isset($name_rel_type_filter['direct'][$rel_type_uuid]) && $name_rel_type_filter['direct'][$rel_type_uuid])
                ||($is_inverse_relation && isset($name_rel_type_filter['inverse'][$rel_type_uuid]) && $name_rel_type_filter['inverse'][$rel_type_uuid])) {


                if (array_search($rel_type_uuid, $non_nec_name_reltype_uuids) !== false &&
                    ((!$is_inverse_relation && $rel_type_uuid != UUID_NAMERELATIONSHIPTYPE_BLOCKING_NAME_FOR)
                        || ($is_inverse_relation && $rel_type_uuid == UUID_NAMERELATIONSHIPTYPE_BLOCKING_NAME_FOR))) {
                    $non_nec_name_rels[] = $name_rel;
                } else if (UUID_NAMERELATIONSHIPTYPE_ORTHOGRAPHIC_VARIANT == $rel_type_uuid) {
                    $orthographic_variants[] = $name_rel;
        ...
        foreach ($name_relations as $name_rel) {
            ...
            $is_inverse_relation = $name_rel->inverse;
            $rel_footnote_key_markup = render_footnote_key(handle_name_relationship_as_footnote($name_rel),'');
            $relationship_markup = name_relationship_new_markup( $name_rel, $current_taxon_uuid, $name_rel->relType, $show_name_cache_only);
            $label = $name_rel->relType;

            $symbol = $name_rel->relType;
            $symbol_markup = '<span class="symbol" title="' . $label . '">' . $symbol . '</span>' . $rel_footnote_key_markup . ' ';
            $relationship_markup = $symbol_markup . $relationship_markup;
            if ($relationship_markup) {
                $render_array['list']['items'][] = markup_to_render_array($relationship_markup,
        ...
        if (count($non_nec_name_rels) > 0) {
            ...
            foreach ($non_nec_name_rels as $name_rel) {
                ...
                $is_inverse_relation = $name_rel->inverse;
                $rel_footnote_key_markup = render_footnote_key(handle_name_relationship_as_footnote($name_rel),'');
                $relationship_markup = name_relationship_new_markup($name_rel, $current_taxon_uuid, $name_type, $show_name_cache_only);
                $label = $name_rel->relType;
        ...
        if (count($orthographic_variants) > 0) {
            ...
            foreach ($orthographic_variants as $name_rel) {
                ...
                //$is_inverse_relation = $current_name_uuid == $name_rel->nameUuid;
                $rel_footnote_key_markup = render_footnote_key(handle_name_relationship_as_footnote($name_rel),'');
                $relationship_markup = name_relationship_markup( $name_rel, $current_taxon_uuid, $show_name_cache_only);
                $label = $name_rel->relType;
                $symbol = cdm_relationship_type_term_symbol($name_rel->type);
                $symbol_markup = '<span class="symbol" title="' . $label . '">' . $symbol . '</span>' . $rel_footnote_key_markup .  ' ';
                $relationship_markup = $symbol_markup . $relationship_markup;
            }
            if (isset($relationship_markup) && $relationship_markup) {
...
function cdm_name_relationships_for_taxon_new($taxon_dto)
{
    //TODO: without WS calls
    /*$from_name_relations = cdm_ws_get(CDM_WS_PORTAL_TAXON_FROM_NAMERELATIONS, $taxon_dto->uuid);
    $to_name_relations = cdm_ws_get(CDM_WS_PORTAL_TAXON_TO_NAMERELATIONS, $taxon_dto->uuid);
    $name_relations = array_merge($from_name_relations, $to_name_relations);*/
    $name_relations = array();
    if (isset_not_empty($taxon_dto->relatedNames) && isset_not_empty($taxon_dto->relatedNames->items)){
        $name_relations = $taxon_dto->relatedNames->items;
    }
    return $name_relations;
}
...
function nameTypeToDTYPE($dtype){
  ...
  if(array_key_exists($dtype, $nameTypeLabelMap)){
 ...
 *    The CDM TypeDesignation entities
 */
function new_type_designations_for_synonymy_group($synonymy_group, $accepted_taxon_name_uuid = null)
{
    if (count($synonymy_group) > 0) {
        $name_uuid = array_pop($synonymy_group)->name->uuid;
    } else {
