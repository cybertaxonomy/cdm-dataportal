...
function cdm_dataportal_search_form_path_for_ws($ws_endpoint) {
  static $form_ws_map = array (
    CDM_WS_PORTAL_TAXON_FIND => "cdm_dataportal/search", CDM_WS_PORTAL_TAXON_FINDBY_DESCRIPTIONELEMENT_FULLTEXT => "cdm_dataportal/search/taxon_by_description" 
  );
...
function cdm_dataportal_search_form_prepare($action_path, $search_webservice, $process = null, $query_field_default_value, $query_field_description) {
  ...
  $form ['ws'] = array (
    '#type' => 'hidden', '#value' => $search_webservice, '#name' => 'ws' 
  );
  
  $form ['query'] = array (
    '#weight' => 0, '#type' => 'textfield', '#size' => 68, '#attributes' => array (
    'title' => $query_field_description 
  ), '#value' => $query_field_default_value, '#description' => $query_field_description 
  );
  ...
  if (! (bool)variable_get('clean_url', '0')) {
    $form ['search'] ['q'] = array (
      '#type' => 'hidden', '#value' => $action_path, '#name' => 'q' 
    );
  }
  
  $form ['submit'] = array (
    '#weight' => 5, '#type' => 'submit', '#name' => '', '#value' => t('Search') 
  );
 ...
 * @return void
 */
function cdm_dataportal_search_taxon_form(&$form_state, $advancedForm = false) {
  global $theme_key;
  ...
  $form ['search'] ['tree'] = array (
    '#weight' => - 1, '#type' => 'hidden', '#value' => get_taxonomictree_uuid_selected() 
  );
  
  $form ['search'] ['pageSize'] = array (
    '#weight' => - 1, '#type' => 'hidden', '#value' => variable_get('cdm_dataportal_search_items_on_page', 25) 
  );
  
  $form ['search'] ['pageNumber'] = array (
    '#weight' => - 1, '#type' => 'hidden', '#value' => 0 
  );
  ...
  if ($advancedForm) {
    ...
    $form ['search'] ['doTaxa'] = array (
      '#weight' => 2, '#type' => 'checkbox', '#title' => t('Search for accepted taxa'), '#value' => $preset_doTaxa 
    );
    $form ['search'] ['doSynonyms'] = array (
      '#weight' => 3, '#type' => 'checkbox', '#title' => t('Search for synonyms'), '#value' => $preset_doSynonyms 
    );
    $form ['search'] ['doMisappliedNames'] = array (
      '#weight' => 4, '#type' => 'checkbox', '#title' => t('Search for misapplied names'), '#value' => $preset_doMisappliedNames 
    );
    $form ['search'] ['doTaxaByCommonNames'] = array (
      '#weight' => 5, '#type' => 'checkbox', '#title' => t('Search for common names'), '#value' => $preset_doTaxaByCommonNames 
    );
    
    // Geographic Range
    $form ['search'] ['geographic_range'] = array (
      '#type' => 'fieldset', '#weight' => 5, '#tree' => true, '#title' => t('Geographic range') 
    );
    
    $form ['search'] ['geographic_range'] ['tdwg_level_select'] = array (
      '#type' => 'radios', '#title' => t('Select a TDWG distribution level and code'), '#default_value' => $tdwg_level_select, '#options' => array (
      t('TDWG level-1, i.e. a continent'), t('TDWG level-2'), t('TDWG level-3, i.e. a country'), t('TDWG level-4') 
    ) 
    );
    ...
    foreach ($tdwg as $key => $tdwg_level) {
      ...
      $form ['search'] ['geographic_range'] ['tdwg_level_' . $key] = array (
        '#type' => 'select', '#title' => t('TDWG level') . ' ' . $key, '#default_value' => $tdwgOptionsSelected, '#multiple' => TRUE, '#options' => $tdwgOptions 
      );
    ...
    $form ['search'] ['doTaxa'] = array (
      '#weight' => - 2, '#type' => 'hidden', '#value' => $preset_doTaxa 
    );
    $form ['search'] ['doSynonyms'] = array (
      '#weight' => - 3, '#type' => 'hidden', '#value' => $preset_doSynonyms 
    );
    $form ['search'] ['doMisappliedNames'] = array (
      '#weight' => - 4, '#type' => 'checkbox', '#title' => t('Misapplied names'), '#value' => $preset_doMisappliedNames 
    );
    $form ['search'] ['doTaxaByCommonNames'] = array (
      '#weight' => - 5, '#type' => 'hidden', '#value' => $preset_doTaxaByCommonNames 
    );
  }
...
function cdm_dataportal_search_taxon_form_advanced() {
  return cdm_dataportal_search_taxon_form($form_state, true);
}
...
function cdm_dataportal_search_taxon_by_description_form() {
  ...
  $form = cdm_dataportal_search_form_prepare('cdm_dataportal/search/results/taxon', CDM_WS_PORTAL_TAXON_FINDBY_DESCRIPTIONELEMENT_FULLTEXT, null, $query_field_default_value, t('Enter the text you wish to search for. The asterisk character * can be used as wildcard. Terms can be combined with \'AND\'. To search for a full phrase enclose the terms in parentheses. For more syntactial options please refer to the ') . l('Apache Lucene - Query Parser Syntax', 'http://lucene.apache.org/core/old_versioned_docs/versions/2_9_1/queryparsersyntax.html', array (
    'attributes' => null, 'query' => null, 'fragment' => null, 'absolute' => TRUE, 'html' => TRUE 
  )));
  ...
  if (module_exists("user") && user_access('administer')) {
    $form ['search'] ['clazz'] = array (
      '#type' => 'select', '#title' => t('Limit to DescriptionElement type'), '#default_value' => $_SESSION ['cdm'] ['search'] ['clazz'], '#options' => cdm_descriptionElementTypes_as_option(true) 
    );
  }
  ...
  if(!isset($_SESSION ['cdm'] ['search'] ['features'])){
    $form ['search'] ['features'] = array (
      '#type' => 'checkboxes', '#title' => t('Limit to selected features'), '#default_value' => $_SESSION ['cdm'] ['search'] ['features'], '#options' => $feature_options 
    );
  } else {
    $form ['search'] ['features'] = array (
      '#type' => 'checkboxes', '#title' => t('Limit to selected features'), '#options' => $feature_options
    );
  }
 ...
 * array();
 * Implementation of #process method call, see form_builder(, $form_state)
 *
