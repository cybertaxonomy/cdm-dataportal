...
function cdm_load_node($nodetype, $uuid, $title) {
  ...
  // try to find node id
  $cdmnode = db_fetch_object(db_query('SELECT nid, cdmtype FROM {node_cdm} WHERE wsuri = \'%s\' AND cdmtype = \'%s\' AND uuid = \'%s\'', variable_get('cdm_webservice_url', NULL), $nodetype, $uuid));
  ...
  if (is_numeric($cdmnode->nid)) {
    ...
    // @WA: $comment_node_disabled is not defined
    // $values['comment'] = variable_get('comment__' . $node->type. $nodetype,
    // $comment_node_disabled);
    $values ['comment'] = variable_get('comment__' . $node->type . $nodetype, false);
    ...
    if (! is_array($result)) {
      ...
      // ---- checking for problems, see
      // http://dev.e-taxonomy.eu/trac/ticket/2964
      if ($nid == 0) {
      ...
      // @WA
      // if(!referer_uri() || !isset(ip_address())){
      if (! referer_uri() || ! (ip_address())) {
      ...
      $node->nid = $nid;
      $hash = md5(variable_get('cdm_webservice_url', NULL) . $uuid); // hash
                                                                       // as a
                                                                       // 32-character
                                                                       // hexadecimal
                                                                       // number.
      db_query('INSERT INTO {node_cdm} (nid, wsuri, hash, cdmtype, uuid) VALUES (%d, \'%s\', \'%s\', \'%s\', \'%s\');', $nid, variable_get('cdm_webservice_url', NULL), $hash, $nodetype, $uuid);
    }
    else {
...
function cdm_add_node_content(&$node, &$content, $weight) {
  ...
  '#value' => '<div id="cdm_dataportal.node">' . $content . '</div>', '#weight' => variable_get('cdm_content_weight', - 1) 
  );
